...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------KurrGanteng'+'KURRTOD--GANS';
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'kurrtod') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'kurrtod')  return res.json(loghandler.invalidKey)
